...
MURDOCK_DOCKER_ARGS=${MURDOCK_DOCKER_ARGS:-"--tmpfs /tmp:size=2g,exec,nosuid"}
...
exec docker run --rm -u $(id -u ${MURDOCK_USER}) \
    ...
    kaspar030/riotdocker:latest murdock_slave --name $MURDOCK_HOSTNAME --queues ${MURDOCK_QUEUES} "$@"
